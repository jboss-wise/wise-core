...
public class ElementBuilderImpl implements ElementBuilder {
    ...
    public Element buildTree(Type type, String name, Object value, boolean nillable) {
        ...
        return buildTree(type, name, value, nillable, null, null,
                Collections.synchronizedMap(new HashMap<Type, ElementImpl>()), new HashSet<Type>());
    }

    private ElementImpl buildTree(Type type, String name, Object obj, boolean nillable, Class<?> scope, String namespace,
            Map<Type, ElementImpl> typeMap, Set<Type> stack) {
    ...
    private ElementImpl buildParameterizedType(ParameterizedType pt, String name, Object obj, Class<?> scope, String namespace,
            Map<Type, ElementImpl> typeMap, Set<Type> stack) {
    ...
    private ElementImpl buildFromClass(Class<?> cl, String name, Object obj, boolean nillable, Map<Type, ElementImpl> typeMap,
            Set<Type> stack) {
        ...
        if (isSimpleType(cl, client)) {
            ...
            final boolean recursionCheck = (request && obj == null); // lazy element disable till we have a value object (an
                                                                     // actual SOAP request won't have cycles)
            ...
            for (Field field : ReflectionUtils.getAllFields(cl)) {
                ...
                ElementImpl element = this.buildTree(field.getGenericType(), fieldName, fieldValue, true, cl, namespace,
                        typeMap, stack);
    ...
    private ElementImpl newElement(Type classType, String name, boolean forceNotNillable, ElementImpl prototype, boolean lazy,
            Map<Type, ElementImpl> treeTypesMap) {
        ElementImpl element = new ElementImpl(false);
        element.setClassType(classType);
        element.setName(name);
        element.setId(generateNewID());
        if (classType instanceof ParameterizedType
                && Collection.class.isAssignableFrom((Class<?>) ((ParameterizedType) classType).getRawType())) {
