...
public class JBossEntityResolver implements EntityResolver {
    ...
    /**
     * A class flag indicating whether an attempt to resolve a systemID as a non-file URL should produce a warning rather than a
     * trace level log msg.
     */
    ...
    static {
        ...
        registerEntity("http://www.ibm.com/webservices/xsd/j2ee_web_services_client_1_1.xsd",
        ...
        registerEntity("http://java.sun.com/xml/ns/javaee/javaee_web_services_client_1_2.xsd",
     ...
     *
     * 1. Check the publicId against the current registered values in the class mapping of entity name to dtd/schema file name.
     * If found, the resulting file name is passed to the loadClasspathResource to locate the file as a classpath resource.
     *
     * 2. Check the systemId against the current registered values in the class mapping of entity name to dtd/schema file name.
     * If found, the resulting file name is passed to the loadClasspathResource to locate the file as a classpath resource.
     *
     * 3. Strip the systemId name down to the simple file name by removing an URL style path elements (myschemas/x.dtd becomes
     * x.dtd), and call loadClasspathResource to locate the simple file name as a classpath resource.
     *
     * 4. Attempt to resolve the systemId as a URL from which the schema can be read. If the URL input stream can be opened this
     * returned as the resolved input.
     *
    ...
    public InputSource resolveEntity(String publicId, String systemId) throws SAXException, IOException {
        ...
        if (publicId != null && systemId != null) {
            ...
            if (registeredSystemId != null && !registeredSystemId.equals(systemId)) {
                ...
                if (trace)
                    log.trace("systemId argument '" + systemId + "' for publicId '" + publicId
                            + "' is different from the registered systemId '" + registeredSystemId
                            + "', resolution will be based on the argument");
            }
        }
     ...
     * @return the InputSource for the schema file found on the classpath, null if the publicId is not registered or found.
     */
    ...
    /**
     * Attempt to use the systemId as a URL from which the schema can be read. This checks to see whether the systemId is a key
     * to an entry in the class entity map.
     *
     * @param systemId - the systemId
     * @param trace - trace level logging flag
     * @return the URL InputSource if the URL input stream can be opened, null if the systemId is not a URL or could not be
     *         opened.
     */
     ...
     * @param systemId - the system ID of DTD or Schema
     * @param trace - trace level logging flag
     * @return the InputSource for the schema file found on the classpath, null if the systemId is not registered or found.
     */
    ...
    /**
     * Look for the resource name on the thread context loader resource path. This first simply tries the resource name as is,
     * and if not found, the resource is prepended with either "dtd/" or "schema/" depending on whether the resource ends in
     * ".dtd" or ".xsd".
     *
    ...
    protected InputStream loadClasspathResource(String resource, boolean trace) {
        ...
        if (url == null) {
            /*
             * Prefix the simple filename with the schema type patch as this is the naming convention for the jboss bundled
             * schemas.
             */
