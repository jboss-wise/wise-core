...
public class WSDLResolver {
    ...
    private void saveWsdlImports(URL parentURL, Definition parentDefinition, Map<String, String> saved, URL wsdlURL)
            throws Exception {
        ...
        while (it.hasNext()) {
            for (Import wsdlImport : (List<Import>) it.next()) {
                ...
                URL targetURL = new URL(baseURI.substring(0, baseURI.lastIndexOf("/") + 1) + "wsdl-" + IDGenerator.nextVal()
                        + ".wsdl");
                ...
                for (Element domElement : getSchemaElements(subdef)) {
                    saveSchemaImports(parentURL, domElement, saved,
                            new URL(baseWsdlURI.substring(0, baseWsdlURI.lastIndexOf("/") + 1) + locationURI));
                }
        ...
        while (it.hasNext()) {
            ...
            if ("import".equals(childElement.getLocalName()) || "include".equals(childElement.getLocalName())) {
                ...
                if (schemaLocation.length() > 0) {
                    ...
                    URL xsdURL = new URL(baseURI.substring(0, baseURI.lastIndexOf("/") + 1) + "schema-" + IDGenerator.nextVal()
                            + ".xsd");
